cmd_list_json() {
  ...
  local first_base_repo base_branch present remote_available remote_commit
  base_commits=()
  i=0
  ...
  while [ $i -lt ${#REPO_NAMES[@]} ]; do
    ...
    if [ -d "$repo_path" ] && git -C "$repo_path" rev-parse --git-dir >/dev/null 2>&1; then
      ...
      if remote_commit=$(git -C "$repo_path" rev-parse --verify --quiet "refs/remotes/origin/$base_branch^{commit}" 2>/dev/null); then
